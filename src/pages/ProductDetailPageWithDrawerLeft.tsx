...
export default function ProductDetailPageWithDrawerLeft() {
    ...
    <RetailerLayout languageSelector={false} cartCount={13}>
      <div id="main" className="relative w-full pb-4 md:pb-0">
        ...
        <div className="retailer-12col-grid mx-auto" style={{ maxWidth: "1920px", paddingLeft: "48px", paddingRight: "48px", paddingTop: "48px", alignItems: "start" }}>
            {/* Compass Drawer - Columns 1-3 (Left side) */}
            {compassState.isPanelOpen && (
              <div 
                className="md:sticky md:top-[48px] md:z-10"
                style={{ gridColumn: "1 / 4", gridRow: "1" }}
              >
                <CompassDrawer position="left" />
              </div>
            )}

            {/* Product Images - Columns 4-11 when drawer open, 1-8 when closed */}
            <div 
              data-test-id="image-section"
              className="grid grid-cols-2"
              style={{ 
                gridColumn: compassState.isPanelOpen ? "4 / 12" : "1 / 9", 
                gridRow: "1", 
                gap: "2px" 
              }}
            >
            ...
            {/* Brand Info and Product Details - Column 12 when drawer open, 9-12 when closed */}
            <div 
              className="w-full md:sticky md:top-[48px] md:z-10 area-product-details"
              style={{ 
                gridColumn: compassState.isPanelOpen ? "12 / 13" : "9 / 13", 
                gridRow: "1", 
                paddingLeft: "48px" 
              }}
            >
